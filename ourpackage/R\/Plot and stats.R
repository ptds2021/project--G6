library(dplyr)
library(ggplot2)
library(devtools) # maybe to put in another file
...
# Function to give info about sustainibility
sustainability <- function(countries_select) {
  
data <- read_csv("data/NFA 2019 public_data.csv") # maybe to remove
...
plot_sust <- ggplot(sustbycountry, 
                         mapping = aes(x = year, 
                                       y = sustainablity, 
                                       fill = factor(country))) +
