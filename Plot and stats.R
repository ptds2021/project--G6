...
countries <- list("France", "Guyana", "Suriname", "Afghanistan")
Sbycountry <- filter(sustain, country %in% countries)
plot_countries <- plot <- ggplot(countries, mapping = aes(x = year, 
                                                           y = sustainablity)) +
  geom_histogram(stat = "identity") + 
  labs(title = paste("Sustainability per year for", countries))
